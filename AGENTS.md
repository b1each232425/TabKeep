...
- 计划中的 `desktop/`：TabKeep 自己的桌面状态，用于承接浏览器扩展无法安全提供的系统级能力。
...
- 不要求用户安装 Pot Desktop。
- 不把 Pot 类功能塞进 OpenWiki；桌面状态属于 TabKeep。
...
- 桌面状态使用 Tauri 2，不使用 Electron。
- 桌面状态默认监听 `http://127.0.0.1:38472`。
...
验收目标：扩展能检测桌面状态是否运行；桌面状态未运行时，现有收藏和标签管理不受影响。
...
- 桌面状态超过原型阶段后，写入类或返回数据类接口优先加 token 保护。
...
- 现有标签分组和笔记保存流程仍可用。
- 桌面状态缺席时，popup 和 background service worker 不崩溃。
- 新增本地端点有基本成功路径和失败路径。
...
- 构建桌面状态基础能力时，不做无关大重构。
- 不确定时，保留现有 TabKeep 行为，把新桌面行为作为可选路径加入。
